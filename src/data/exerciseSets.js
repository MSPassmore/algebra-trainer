export const exerciseSets = [
  {
    id: "version-1",
    title: "Version 1",
    description: "Introductory algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}2x + y &= 7 \\\\ x - y &= 1\\end{aligned}",
        fields: ["x", "y"],
        answers: [8 / 3, 5 / 3],
        solution: "x = \\frac{8}{3}, \\quad y = \\frac{5}{3}",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A shop sells pens and notebooks. 2 pens and 1 notebook cost 7 CHF. 1 pen and 1 notebook cost 4 CHF. Let x be the price of a pen and y the price of a notebook.",
        question: "\\begin{aligned}2x + y &= 7 \\\\ x + y &= 4\\end{aligned}",
        fields: ["x", "y"],
        answers: [3, 1],
        solution: "x = 3, \\quad y = 1",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "x^2 - 5x + 6 = 0",
        fields: ["x₁", "x₂"],
        answers: [2, 3],
        solution: "x = 2 \\quad \\text{or} \\quad x = 3",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "2x(x-2)+10 = x(x+3)",
        fields: ["x₁", "x₂"],
        answers: [2, 5],
        solution: "x = 2 \\quad \\text{or} \\quad x = 5",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{x+7}=4",
        fields: ["x"],
        answers: [9],
        solution: "x = 9",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "2^x = 16",
        fields: ["x"],
        answers: [4],
        solution: "x = 4",
      },
    ],
  },
  {
    id: "version-2",
    title: "Version 2",
    description: "Slightly harder algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}3x + y &= 11 \\\\ x - y &= 1\\end{aligned}",
        fields: ["x", "y"],
        answers: [3, 2],
        solution: "x = 3, \\quad y = 2",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A cinema sells adult tickets and student tickets. 1 adult ticket and 2 student tickets cost 24 CHF. 2 adult tickets and 1 student ticket cost 27 CHF. Let x be the price of an adult ticket and y the price of a student ticket.",
        question: "\\begin{aligned}x + 2y &= 24 \\\\ 2x + y &= 27\\end{aligned}",
        fields: ["x", "y"],
        answers: [18, 3],
        solution: "x = 18, \\quad y = 3",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "x^2 + 3x - 10 = 0",
        fields: ["x₁", "x₂"],
        answers: [2, -5],
        solution: "x = 2 \\quad \\text{or} \\quad x = -5",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "x(3x+3)+14 = 2x(x-3)",
        fields: ["x₁", "x₂"],
        answers: [-7, -2],
        solution: "x = -7 \\quad \\text{or} \\quad x = -2",
      },
      {
        type: "multi",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{x^2+9}-3=2",
        fields: ["x₁", "x₂"],
        answers: [4, -4],
        solution: "x = 4 \\quad \\text{or} \\quad x = -4",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "3^x - 3^2 = 18",
        fields: ["x"],
        answers: [3],
        solution: "x = 3",
      },
    ],
  },
  {
    id: "version-3",
    title: "Version 3",
    description: "Intermediate algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}3x - 2y &= 8 \\\\ x + y &= 6\\end{aligned}",
        fields: ["x", "y"],
        answers: [4, 2],
        solution: "x = 4, \\quad y = 2",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A theatre sold 12 tickets. Adult tickets cost 10 CHF and child tickets cost 5 CHF. The total income was 85 CHF. Let x be the number of adult tickets and y the number of child tickets.",
        question: "\\begin{aligned}x + y &= 12 \\\\ 10x + 5y &= 85\\end{aligned}",
        fields: ["x", "y"],
        answers: [5, 7],
        solution: "x = 5, \\quad y = 7",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "x^2 - 7x + 12 = 0",
        fields: ["x₁", "x₂"],
        answers: [3, 4],
        solution: "x = 3 \\quad \\text{or} \\quad x = 4",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "(x+7)(x-3)+24 = 2x(x+3)",
        fields: ["x₁", "x₂"],
        answers: [-3, 1],
        solution: "x = -3 \\quad \\text{or} \\quad x = 1",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation. Round to 2 decimal places if necessary.",
        question: "\\sqrt{2x+5}+1=x",
        fields: ["x"],
        answers: [2 + 2 * Math.sqrt(2)],
        solution: "x = 2 + 2\\sqrt{2} \\approx 4.83",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "4^{x+1}=32",
        fields: ["x"],
        answers: [1.5],
        solution: "x = 1.5",
      },
    ],
  },
  {
    id: "version-4",
    title: "Version 4",
    description: "Intermediate algebra with negative numbers.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}2x - y &= 9 \\\\ x + 3y &= 1\\end{aligned}",
        fields: ["x", "y"],
        answers: [4, -1],
        solution: "x = 4, \\quad y = -1",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "At a school event, 2 sandwiches and 3 drinks cost 31 CHF. 1 sandwich and 2 drinks cost 18 CHF. Let x be the price of a sandwich and y the price of a drink.",
        question: "\\begin{aligned}2x + 3y &= 31 \\\\ x + 2y &= 18\\end{aligned}",
        fields: ["x", "y"],
        answers: [8, 5],
        solution: "x = 8, \\quad y = 5",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "2x^2 - 5x - 3 = 0",
        fields: ["x₁", "x₂"],
        answers: [3, -0.5],
        solution: "x = 3 \\quad \\text{or} \\quad x = -\\frac{1}{2}",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "(x+3)^2 = 2(x+7)",
        fields: ["x₁", "x₂"],
        answers: [-5, 1],
        solution: "x = -5 \\quad \\text{or} \\quad x = 1",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{x^2-4x+13}=x+1",
        fields: ["x"],
        answers: [2],
        solution: "x = 2",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "2\\cdot5^{x-1}-7=43",
        fields: ["x"],
        answers: [3],
        solution: "x = 3",
      },
    ],
  },
  {
    id: "version-5",
    title: "Version 5",
    description: "More challenging algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}x + y &= 3 \\\\ 2x - y &= -9\\end{aligned}",
        fields: ["x", "y"],
        answers: [-2, 5],
        solution: "x = -2, \\quad y = 5",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "The sum of two numbers is 18. The difference between the larger number and the smaller number is 4. Let x be the larger number and y the smaller number.",
        question: "\\begin{aligned}x + y &= 18 \\\\ x - y &= 4\\end{aligned}",
        fields: ["x", "y"],
        answers: [11, 7],
        solution: "x = 11, \\quad y = 7",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "x^2 + x - 20 = 0",
        fields: ["x₁", "x₂"],
        answers: [4, -5],
        solution: "x = 4 \\quad \\text{or} \\quad x = -5",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "x(5x-1)=3(x+2)(x-2)+18",
        fields: ["x₁", "x₂"],
        answers: [-1.5, 2],
        solution: "x = -1.5 \\quad \\text{or} \\quad x = 2",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation. Round to 2 decimal places if necessary.",
        question: "\\sqrt{3x+10}=x-2",
        fields: ["x"],
        answers: [(7 + Math.sqrt(73)) / 2],
        solution: "x = \\frac{7+\\sqrt{73}}{2} \\approx 7.77",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation. Round to 2 decimal places if necessary.",
        question: "e^{2x-1}+4=13",
        fields: ["x"],
        answers: [(Math.log(9) + 1) / 2],
        solution: "x = \\frac{\\ln(9)+1}{2} \\approx 1.60",
      },
    ],
  },
  {
    id: "version-6",
    title: "Version 6",
    description: "Mixed algebra practice with fractions.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}4x + 2y &= 8 \\\\ 2x - y &= -2\\end{aligned}",
        fields: ["x", "y"],
        answers: [0.5, 3],
        solution: "x = \\frac{1}{2}, \\quad y = 3",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A restaurant sells burgers and drinks. 3 burgers and 2 drinks cost 22 CHF. 2 burgers and 5 drinks cost 22 CHF. Let x be the price of a burger and y the price of a drink.",
        question: "\\begin{aligned}3x + 2y &= 22 \\\\ 2x + 5y &= 22\\end{aligned}",
        fields: ["x", "y"],
        answers: [6, 2],
        solution: "x = 6, \\quad y = 2",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "3x^2 - 8x + 4 = 0",
        fields: ["x₁", "x₂"],
        answers: [2, 2 / 3],
        solution: "x = 2 \\quad \\text{or} \\quad x = \\frac{2}{3}",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation. Round to 2 decimal places if necessary.",
        question: "(x+2)^2-(x+2)(3-2x)=-4",
        fields: ["x₁", "x₂"],
        answers: [-1, -2 / 3],
        solution: "x = -1 \\quad \\text{or} \\quad x = -\\frac{2}{3}",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{x+10}=x-2",
        fields: ["x"],
        answers: [6],
        solution: "x = 6",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "3^{x+1}=81",
        fields: ["x"],
        answers: [3],
        solution: "x = 3",
      },
    ],
  },
  {
    id: "version-7",
    title: "Version 7",
    description: "Advanced mixed algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}2x - y &= 6 \\\\ 4x + 3y &= 7\\end{aligned}",
        fields: ["x", "y"],
        answers: [2.5, -1],
        solution: "x = 2.5, \\quad y = -1",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A teacher buys 20 items: rulers and erasers. A ruler costs 3 CHF and an eraser costs 1 CHF. The total cost is 44 CHF. Let x be the number of rulers and y the number of erasers.",
        question: "\\begin{aligned}x + y &= 20 \\\\ 3x + y &= 44\\end{aligned}",
        fields: ["x", "y"],
        answers: [12, 8],
        solution: "x = 12, \\quad y = 8",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "4x^2 - 13x + 3 = 0",
        fields: ["x₁", "x₂"],
        answers: [3, 0.25],
        solution: "x = 3 \\quad \\text{or} \\quad x = \\frac{1}{4}",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "(x-5)(2x+5)+5x+8=(x+2)^2",
        fields: ["x₁", "x₂"],
        answers: [-3, 7],
        solution: "x = -3 \\quad \\text{or} \\quad x = 7",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{5x+1}=x-1",
        fields: ["x"],
        answers: [7],
        solution: "x = 7",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "5^{2x-1}=125",
        fields: ["x"],
        answers: [2],
        solution: "x = 2",
      },
    ],
  },
  {
    id: "version-8",
    title: "Version 8",
    description: "Most challenging mixed algebra practice.",
    exercises: [
      {
        type: "system",
        title: "Question 1",
        instruction: "Solve the system.",
        question: "\\begin{aligned}3x + 2y &= 5 \\\\ x + y &= 3\\end{aligned}",
        fields: ["x", "y"],
        answers: [-1, 4],
        solution: "x = -1, \\quad y = 4",
      },
      {
        type: "system",
        title: "Question 2",
        instruction:
          "A sports club has 30 members. There are adults and juniors. Each adult pays 3 CHF and each junior pays 2 CHF. Altogether they pay 70 CHF. Let x be the number of adults and y the number of juniors.",
        question: "\\begin{aligned}x + y &= 30 \\\\ 3x + 2y &= 70\\end{aligned}",
        fields: ["x", "y"],
        answers: [10, 20],
        solution: "x = 10, \\quad y = 20",
      },
      {
        type: "multi",
        title: "Question 3",
        instruction: "Solve the quadratic equation.",
        question: "2x^2 + x - 15 = 0",
        fields: ["x₁", "x₂"],
        answers: [2.5, -3],
        solution: "x = 2.5 \\quad \\text{or} \\quad x = -3",
      },
      {
        type: "multi",
        title: "Question 4",
        instruction: "Expand first, then solve the quadratic equation.",
        question: "(x-4)^2+(x-2)(x+6)=7x+10",
        fields: ["x₁", "x₂"],
        answers: [-0.5, 6],
        solution: "x = -0.5 \\quad \\text{or} \\quad x = 6",
      },
      {
        type: "single",
        title: "Question 5",
        instruction: "Solve the root equation.",
        question: "\\sqrt{x^2+2x+10}=x+4",
        fields: ["x"],
        answers: [-1],
        solution: "x = -1",
      },
      {
        type: "single",
        title: "Question 6",
        instruction: "Solve the power equation.",
        question: "2^{3x-1}=32",
        fields: ["x"],
        answers: [2],
        solution: "x = 2",
      },
    ],
  },
];
